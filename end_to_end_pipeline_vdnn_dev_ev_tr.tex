...
% in the abstract
\begin{abstract}
Video annotation as a means to gather training data-set is proven to be beneficial in the progress of machine learning and deep learning techniques. Recently, front-ends to achieve easy and efficient (deep neural network) DNN prototyping has gained much attention in the deep learning domain. A system where one can load DNN models under development, study or evaluation and apply them directly on the video to learn model accuracy is not readily available. Such close integration of automatic annotation and evaluation is the best way to rapidly generate ground truth and evaluate models on a high level. With this paper, we discuss a system implementing the end-to-end pipeline which help achieve faster ground truth generation, evaluation, and research. The system inherently support Darknet with YOLO and tensorflow based object detection and classification models like Faster-RCNN (the system allows easy integration with other models). 
...
% no keywords
\begin{IEEEkeywords}
Video annotation, pipelined-supervised learning, interactive annotation tool, ground truth generation.
\end{IEEEkeywords}
...
An interactive video annotation tool where one can easily annotate objects, visualize the evaluation metric and work on rapid prototyping is the holy grail of model evaluation, application development and research. This paper presents a system which enable such pipelined ground truth generation and evaluation of multiple models against the generated ground truth. The system keeps track of the evaluation metric in terms of object classification accuracy, bounding box precision and undetected objects that are manually added. This evaluation metric could even help the researcher to easily track weak spots and improve on the learning algorithm. The comprehensive evaluation module deals with automatic model evaluation with easy to understand representation of pitfalls, accuracy measures and errors on a per class, per frame, per video basis. \par
The system discussed in this paper is developed as part of a research project aimed at creating an ecosystem that supports easy ground truth generation, model evaluation and application development. The tool helps developers to quickly generate ground truth for classes of interest, evaluate available DNN models for those classes and . \par
